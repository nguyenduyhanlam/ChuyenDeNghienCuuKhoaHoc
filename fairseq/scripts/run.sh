...
train_model(){   
    ...
    python $codedir/train.py $workdir/processed_data \
        -a transformer_wmt_en_de_big --optimizer adam --lr 0.0005 -s $src -t $tgt \
        --label-smoothing 0.1 --dropout 0.3 --max-tokens 4500  \
        ...
        --share-all-embeddings --alignment-task 'vanilla' --save-interval 1 --fp16 \
...
train_AET_transformer(){   
    ...
    python $codedir/train.py $workdir/processed_data \
        ...
        --adam-betas '(0.9, 0.98)' --save-dir $modeldir --log-interval 1000 \
        --share-all-embeddings --save-interval 1 --fp16 \
...
extract_bidirectional_alignment(){    
    ...
    for split in 'train' 'valid'; do
        ...
        python $codedir/generate_align.py $workdir/processed_data -s $src -t $tgt --path $Model/checkpoint_best.pt \
        ...
        python $codedir/generate_align.py $workdir/processed_data -s $tgt -t $src --path $Model/checkpoint_best.pt \
            --max-tokens 8000 --beam 1 --remove-bpe --quiet --print-vanilla-alignment --decoding-path $resdir  \
            --alignment-task vanilla --gen-subset $split --alignment-layer 2 --set-shift 

        method="grow-diagonal"
        python $codedir/scripts/aer/combine_bidirectional_alignments.py $resdir/$split.${src}2${tgt}.align $resdir/$split.${tgt}2${src}.align --method $method > $resdir/$split.${src}2${tgt}.bidir.align
    done 
...
calculate_AET_att_AER(){   
    ...
    python $codedir/extract_dual_alignment.py $workdir/processed_data -s $src -t $tgt --path $Model/checkpoint_best.pt \
    ...
    python $codedir/scripts/aer/sentencepiece_to_word_alignments.py --src $bpe/test.$src \
    ...
    python $codedir/scripts/aer/sentencepiece_to_word_alignments.py --src $bpe/test.$tgt \
    ...
    python $codedir/scripts/aer/aer.py ${ref_align} $resdir/$split.${src}2${tgt}.dualf.raw.align --fAlpha 0.5 --oneRef 
    python $codedir/scripts/aer/aer.py ${ref_align} $resdir/$split.${src}2${tgt}.dualb.raw.align --fAlpha 0.5 --oneRef --reverseHyp
    python $codedir/scripts/aer/aer.py ${ref_align} $resdir/$split.${src}2${tgt}.bidir.aet.align --fAlpha 0.5 --oneRef 
}
...
calculate_shift_att_AER(){   
    ...
    method="grow-diagonal"
    python $codedir/scripts/aer/combine_bidirectional_alignments.py $resdir/$split.${src}2${tgt}.raw.align  $resdir/$split.${tgt}2${src}.raw.align --method $method > $resdir/$split.${src}2${tgt}.bidir.align

    echo "=====calculate AER for shift-att method=========="   
    python $codedir/scripts/aer/aer.py ${ref_align} $resdir/$split.${src}2${tgt}.raw.align --fAlpha 0.5 --oneRef 
    python $codedir/scripts/aer/aer.py ${ref_align} $resdir/$split.${tgt}2${src}.raw.align --fAlpha 0.5 --oneRef --reverseHyp
    python $codedir/scripts/aer/aer.py ${ref_align} $resdir/$split.${src}2${tgt}.bidir.align --fAlpha 0.5 --oneRef 
}
...
codedir=/content/ChuyenDeNghienCuuKhoaHoc/fairseq
